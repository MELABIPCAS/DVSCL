...
class NCEAverage(nn.Module):
    ...
    def __init__(self, inputSize, outputSize, K, T=0.07, momentum=0.5, use_softmax=False):
        ...
        self.register_buffer('params', torch.tensor([K, T, -1, -1, momentum]))
        stdv = 1. / math.sqrt(inputSize / 3)
        self.register_buffer('memory_rgb', torch.rand(outputSize, inputSize).mul_(2 * stdv).add_(-stdv))
        self.register_buffer('memory_depth', torch.rand(outputSize, inputSize).mul_(2 * stdv).add_(-stdv))

    def forward(self, rgb, depth, y, idx=None):
        K = int(self.params[0].item())
        T = self.params[1].item()
        Z_rgb = self.params[2].item()
        Z_depth = self.params[3].item()

        momentum = self.params[4].item()
        batchSize = rgb.size(0)
        outputSize = self.memory_rgb.size(0)
        inputSize = self.memory_rgb.size(1)
        ...
        if idx is None:
            ...
            idx.select(1, 0).copy_(y.data)
        # sample
        weight_rgb = torch.index_select(self.memory_rgb, 0, idx.view(-1)).detach()
        weight_rgb = weight_rgb.view(batchSize, K + 1, inputSize)
        out_depth = torch.bmm(weight_rgb, depth.view(batchSize, inputSize, 1))
        # sample
        weight_depth = torch.index_select(self.memory_depth, 0, idx.view(-1)).detach()
        weight_depth = weight_depth.view(batchSize, K + 1, inputSize)
        out_rgb = torch.bmm(weight_depth, rgb.view(batchSize, inputSize, 1))

        if self.use_softmax:
            out_depth = torch.div(out_rgb, T)
            out_rgb = torch.div(out_depth, T)
            out_rgb = out_rgb.contiguous()
            out_depth = out_depth.contiguous()
        else:
            out_depth = torch.exp(torch.div(out_depth, T))
            out_rgb = torch.exp(torch.div(out_rgb, T))
            # set Z_0 if haven't been set yet,
            # Z_0 is used as a constant approximation of Z, to scale the probs
            if Z_rgb < 0:
                self.params[2] = out_rgb.mean() * outputSize
                Z_rgb = self.params[2].clone().detach().item()
                print("normalization constant Z_rgb is set to {:.1f}".format(Z_rgb))
            if Z_depth < 0:
                self.params[3] = out_depth.mean() * outputSize
                Z_depth = self.params[3].clone().detach().item()
                print("normalization constant Z_depth is set to {:.1f}".format(Z_depth))
            # compute out_rgb, out_depth
            out_rgb = torch.div(out_rgb, Z_rgb).contiguous()
            out_depth = torch.div(out_depth, Z_depth).contiguous()

        # # update memory
        with torch.no_grad():
            rgb_pos = torch.index_select(self.memory_rgb, 0, y.view(-1))
            rgb_pos.mul_(momentum)
            rgb_pos.add_(torch.mul(rgb, 1 - momentum))
            rgb_norm = rgb_pos.pow(2).sum(1, keepdim=True).pow(0.5)
            updated_rgb = rgb_pos.div(rgb_norm)
            self.memory_rgb.index_copy_(0, y, updated_rgb)

            depth_pos = torch.index_select(self.memory_depth, 0, y.view(-1))
            depth_pos.mul_(momentum)
            depth_pos.add_(torch.mul(depth, 1 - momentum))
            depth_norm = depth_pos.pow(2).sum(1, keepdim=True).pow(0.5)
            updated_depth = depth_pos.div(depth_norm)
            self.memory_depth.index_copy_(0, y, updated_depth)
