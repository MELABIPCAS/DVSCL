...
class ResNetV1(nn.Module):
    def __init__(self, name='resnet18'):
        ...
        if name == 'resnet50':
            self.rgb_to_d = resnet50(in_channel=3, width=0.5)
            self.d_to_rgb = resnet50(in_channel=1, width=0.5)
        elif name == 'resnet18':
            self.rgb_to_d = resnet18(in_channel=1, width=0.5)  # 1 channel r
            self.d_to_rgb = resnet18(in_channel=1, width=0.5)  # 1 channel g
        elif name == 'resnet101':
            self.rgb_to_d = resnet101(in_channel=3, width=0.5)
            self.d_to_rgb = resnet101(in_channel=1, width=0.5)
        else:
            raise NotImplementedError('model {} is not implemented'.format(name))

    def forward(self, x, layer=7):
        # 做rgb内部时 这三个三选一；做 灰度+深度时下面三个都隐藏
        # x = x[:, :2, :, :]  # 取前2个通道 #2 channel rg
        # x = x[:, 1:, :, :]  # 取后个通道 #2 channel gb
        # x = x[:, [0, 2], :, :] # 取第1和第3通道 #2 channel rb
        rgb, d = torch.split(x, [1, 1], dim=1)
...
"""class ResNetV1(nn.Module):
    def __init__(self, name='resnet50'):
        super(ResNetV1, self).__init__()
        if name == 'resnet50':
            self.rgb = resnet50(in_channel=3)
        elif name == 'resnet18':
            self.rgb = resnet18(in_channel=3)
        elif name == 'resnet101':
            self.rgb = resnet101(in_channel=3)
        else:
            raise NotImplementedError('model {} is not implemented'.format(name))

    def forward(self, x, layer=7):
        feat_rgb = self.rgb(x, layer)
        return feat_rgb"""
...
class MyResNetsCMC(nn.Module):
    def __init__(self, name='resnet18v1'):
