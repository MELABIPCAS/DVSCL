...
class ResNetV1(nn.Module):
    def __init__(self, name='resnet50'):
        ...
        if name == 'resnet50':
            ...
            self.rgb_to_d = resnet18(in_channel=3, width=0.5)
            # self.d_to_rgb = resnet18(in_channel=1, width=0.5)  # 深度
            self.d_to_rgb = resnet18(in_channel=3, width=0.5)  # 光流
        elif name == 'resnet101':
    ...
    def forward(self, x, layer=7):
        # rgb, d = torch.split(x, [3, 1], dim=1)  # 深度
        rgb, d = torch.split(x, [3, 3], dim=1)  # 光流
